...
def get_Train_Features(file_path, display_plot=False):
    ...
    for fname in filenames:
        ...
        # print np.sum(hist[0]), np.sum(hist[0][:np.where(hist[1]==200)[0][0]])
...
# P-tile method, for th = 200 is approximately p = 0.05
def find_Threshold(hist):
    p = 0.05
    g = 0
    for i in range(len(hist[0])):
        if float(np.sum(hist[0][:i])) / float(np.sum(hist[0])) >= p:
            g = i
            break
    index_min = np.min(hist[0][g-1:g+2])
    for i in range(g-1, g+2):
        if hist[0][i] == index_min:
            return hist[1][i]
    # default
    return 200
