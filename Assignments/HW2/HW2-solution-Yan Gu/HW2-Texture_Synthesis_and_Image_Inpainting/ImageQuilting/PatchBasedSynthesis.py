...
def main(arg):
    input_path = "ImageQuilting/images/"
    output_path = "ImageQuilting/results/"
    #Image Loading and initializations
    InputName = str(input_path + arg[1])
    ...
    PatchSize = int(arg[2])
    OverlapWidth = int(arg[3])
    InitialThresConstant = float(arg[4])
    ...
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imwrite(str(output_path + arg[1]), img)
